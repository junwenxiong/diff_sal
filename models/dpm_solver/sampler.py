...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, img=None, cond=None):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(x, t_input, img, **model_kwargs)
        else:
    ...
    def model_fn(x, t_continuous, img=None):
        """
        The noise predicition model function that is used for DPM-Solver.
        """
        if guidance_type == "uncond":
            return noise_pred_fn(x, t_continuous, img)
...
class DPM_Solver:
    def __init__(
        ...
        """
        self.model = lambda x, t, img: model_fn(x, t.expand((x.shape[0])), img)  # 这就是一个代理函数，不是真正的model，参数需要与之前的一致
        # self.model = lambda x, t: model_fn(x, t.expand((x.shape[0])))  # 这就是一个代理函数，不是真正的model，参数需要与之前的一致
    ...
    def noise_prediction_fn(self, x, t, img=None):
        """
        Return the noise prediction model.
        """
        return self.model(x, t, img=img)

    def data_prediction_fn(self, x, t, img=None):
        """
        Return the data prediction model (with corrector).
        """
        noise = self.noise_prediction_fn(x, t, img=img)
    ...
    def model_fn(self, x, t, img=None):
        """
        Convert the model to the noise prediction model or the data prediction model. 
        """
        if self.algorithm_type == "dpmsolver++":
            return self.data_prediction_fn(x, t, img=img)
        else:
            return self.noise_prediction_fn(x, t, img=img)
    ...
    def denoise_to_zero_fn(self, x, s, img=None):
        """
        Denoise at the final step, which is equivalent to solve the ODE from lambda_s to infty by first-order discretization. 
        """
        return self.data_prediction_fn(x, s, img=img)
    ...
    def sample(self, x, img=None, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        method='multistep', lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver',
        atol=0.0078, rtol=0.05, return_intermediate=False,
    ):
        """
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                t = timesteps[step]
                t_prev_list = [t]
                model_prev_list = [self.model_fn(x, t, img)]
                if self.correcting_xt_fn is not None:
                ...
                for step in range(1, order):
                    ...
                    model_prev_list.append(self.model_fn(x, t, img))
                ...
                for step in range(order, steps + 1):
                    ...
                    if step < steps:
                        model_prev_list[-1] = self.model_fn(x, t, img)
            ...
            if denoise_to_zero:
                ...
                x = self.denoise_to_zero_fn(x, t, img=img)
                if self.correcting_xt_fn is not None:
